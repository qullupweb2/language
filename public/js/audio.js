...
function createDownloadLink(blob) {
	...
	reader.onloadend = function () {
		 $('.course-tests__test-buttons [name="audio"]').val(reader.result);
		$('#res-fr').submit();
		//var dataString = 'audiosrc=' + base64;

		var test_oral_id = document.getElementById("stopButton");
		var dataString = reader.result;

		// $.ajax({
		// 	type: "POST",
		// 	url: "/proccessing_exam/post_oral",
		// 	data: 'audio_test='+dataString+'&test_oral_id='+test_oral_id+'&_token='+$('[name="_token"]').val(),
		// 	cache: false,
		// 	headers: {
