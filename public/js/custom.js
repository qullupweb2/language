$(document).ready(function() {
    ...
    if(IsSafari()) {
        ...
        $('.datepicker').attr('type', 'date').attr('value','1999-01-01').attr('format', 'd.m.Y');
    }
    ...
    $('select.orig-select').change(function () {
        var start_date = $(this).find('option:selected').data('date');
    ...
    $('.show_select').change(function() {
        ...
        if(checked) {
            $(this).parent().next().show().attr('name', 'whenStart[]');
            ...
            $(this).parent().next().hide().attr('name', '');
            $(this).parent().next().next().next().hide();
            $('.show_select2').prop('checked', false);
            $(this).parent().next().next().next().next().hide().attr('name', '');
    ...
    $('.show-field').change(function () {
        ...
        if(checked) {
            ...
            $('.ifrom').hide();
            $('input[name="passport_data"]').val('');
        }
    });

    $('select[aria-describedby="whenStartHelp"]').change(function() {
        $('.btn-information').attr('data-content', $(this).attr('data-pricetext'));
    });
})
